...
public class RealizaPagamento implements ActionListener {
    ...
    public RealizaPagamento(Menu tela, JTextField cpf, JTextField numConta, String nomeConta, float val, Cliente cliente) {
        this.tela = tela;
        this.cpf = cpf;
        ...
        try {
            ...
            for (Cliente c : tela.getClientes()) {
                if (c.getTipo().equals("J")) {
                    ...
                    if (p.getCnpj().equals(cpf.getText()) && p.getConta().getNumero().equals(numConta.getText()) && p.getNome().equals(nomeConta)) {
